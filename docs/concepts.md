...
| **Artifact** | A structured document a skill produces — for example `requirements.md` or `user-stories.md`. Always plain Markdown so it's easy to read and review. |
...
| **Workflow** | The recommended order of skills (analyze → specify → write stories → render). BA-Kit can always tell you what's sensible to run next. |
