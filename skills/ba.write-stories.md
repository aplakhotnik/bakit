...
inputs: "The approved artifacts/requirements.md in the active task"
...
3. **Generate stories.** For each requirement, write one or more well-formed user stories
   ("As a `<role>`, I want `<capability>` so that `<benefit>`") with testable acceptance
   criteria in Given/When/Then form.
4. **Trace.** Each story's Traceability line lists the requirement id(s) it satisfies; populate
   the `derived_from` front-matter with the full set of covered requirement ids (non-empty).
5. **Draft the artifact.** Populate `templates/artifacts/user-stories.md`; set front-matter
   `id`, `title`, `status: draft`, `created`/`updated`.
6. **Present for review.** Editable draft; do not self-approve.
